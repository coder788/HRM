...
export class EditZone extends Component {
	...
	componentDidMount() {
		...
		if (this.props.match.params.id) {
			apiService.getZone(this.props.match.params.id).then(resp => {
				this.setState({
					loading: false,
				})
	...
	onFinish = values => {
		...
		apiService.updateZone(params).then(resp => {
				...
				>
					<PageHeaderAlt className="bg-white border-bottom" overlap>
						<div className="container">
							<Flex className="py-2" mobileFlex={false} justifyContent="between" alignItems="center">
								<h2 className="mb-3">Edit Zone</h2>
								<div className="mb-3">
					...
					<div className="container" style={{marginTop: 100}}>
						<Spin spinning={loading}>
							<ZoneForm style={{marginTop: 100}} mode="EDIT" bordered={true} saveBtn={false} />
						</Spin>
					</div>
				</Form>
			</>
