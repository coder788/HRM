...
export class Department extends Component {
    ...
    if (this.props.profileid) {
      apiService.getSystemModules(this.props.profileid).then((resp) => {
        ...
        for (let i = 0; i < resp.length; i++) {
          ...
          if (resp[i].actions) {
            resp[i].actions.map(
              (action) => (checks[action.id] = action.allowed)
            );
          }
        }
        this.setState({
  ...
  toggleAll(e) {
    let hide = [];
    this.state.hide.map((state, index) => {
      hide[index] = !state;
    });
    this.setState({
      hide: hide,
  ...
  enableAll(e) {
    let checks = [];
    this.state.checks.map((state, index) => {
      checks[index] = true;
    });
    this.setState({
      checks: checks,
          ...
          renderItem={(module) => (
            <List.Item>
              ...
              >
                <div className="d-flex align-items-center">
                  ...
                  <div className="ml-3">
                    ...
                    {module.actions ? (
                      <p
                        onClick={(e) =>
                          this.showModuleActions(e, module.parent.id)
                        }
                        className="cursor-pointer"
                      >
                        Have {module.actions?.length} actions. click here to
                        customize.
                      </p>
                    ) : (
                      ""
                ...
                <div className="ml-3">
                  <Switch
                    checked={checks[module.parent.id]}
                    key={`module-${module.parent.id}`}
                    onChange={(state) =>
                      this.checkboxChanged(state, module.parent.id)
                    }
                  />
                </div>
              </Flex>
              {module.actions ? (
                <Row
                  className={
                    hide[module.parent.id]
                      ? "ant-permissions-checks d-none"
                      : "ant-permissions-checks"
                  }
                ...
                >
                  {module.actions.map((action) => (
                    <Col key={`module-col-${action.id}`} span={12}>
                      <Switch
                        size="default"
                        checked={checks[action.id]}
                        key={`module-action-${action.id}`}
                        onChange={(state) =>
                          this.checkboxChanged(state, action.id)
                        }
                      />{" "}
                      <span className="ant-check-title">{action.title}</span>
                    </Col>
                  ))}
                </Row>
              ) : (
